...
- **Lines over 400 characters are skipped by the indexer** (minified bundles, one-line builds) — symbols and imports on those lines never enter the graph. The `orphans` and `broken` answers disclose the skipped count so a missing edge is never presented as certainty.
